...
MATMUL_SHAPES = [*FULL_MATMUL_SHAPES, *TINY_MATMUL_SHAPE_CASES]
...
def matmul_shapes(format):
    if format.input_format.is_mx_format() or format.output_format.is_mx_format():
        return FULL_MATMUL_SHAPES
    return MATMUL_SHAPES


def _is_full_matmul_shape(input_tile_dimensions):
    return input_tile_dimensions in FULL_MATMUL_SHAPES
...
@pytest.mark.quasar
@parametrize(
    marks=lambda input_tile_dimensions: (
        [] if _is_full_matmul_shape(input_tile_dimensions) else pytest.mark.nightly
    ),
    input_tile_dimensions=runtime(matmul_shapes),
    format=MATMUL_FORMAT,
    math_fidelity=lambda format, input_tile_dimensions: (
        matmul_math_fidelities(format)
        if _is_full_matmul_shape(input_tile_dimensions)
        else [MathFidelity.LoFi]
    ),
    dest_sync_mode=lambda input_tile_dimensions: (
