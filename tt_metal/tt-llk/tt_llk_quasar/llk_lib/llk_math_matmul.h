...
struct _llk_math_matmul_execution_geometry_t
{
    ...
    std::uint8_t num_k_faces;
    std::uint16_t dst_tile_row_increment;
};

/**
 * @brief Derives the output layout, replay window, and destination strides for matmul.
 *
 ...
 * @param geometry: Replay and destination geometry from @ref _llk_math_matmul_execution_geometry_.
 */
...
inline void _llk_math_matmul_addrmod_(const std::uint8_t ct_dim, const std::uint8_t rt_dim, const _llk_math_matmul_execution_geometry_t& geometry)
{
    ...
    const std::int32_t src_b_row_stride = geometry.face_rows * geometry.num_k_faces;

    // The address modifiers compact the active output axes in 8-row-pass, column, and row order.
    ...
    const bool has_k           = geometry.num_k_faces > 1;
    const std::int32_t k_src_a = has_k ? MAX_FACE_C_DIM * geometry.output_shape.num_faces_c_dim : 0;
    // Full-tile k_src_b is -16, encoded as 48 in 6-bit two's complement.
    // The SrcB counter wraps modulo 64, so CR 32 plus 48 becomes 16.
    ...
    addr_mod_t {
        ...
        .srcb = {.incr = static_cast<std::uint8_t>(0x3F & x_src_b), .clr = 0, .cr = 0},
    ...
    // SrcB CR discards the X step.
    addr_mod_t {
        .srca = {.incr = static_cast<std::uint8_t>(y_axis_src_a), .clr = 0, .cr = 0},
        .srcb = {.incr = static_cast<std::uint8_t>(0x3F & y_axis_src_b), .clr = 0, .cr = has_y},
    ...
    addr_mod_t {
        ...
        .srcb = {.incr = static_cast<std::uint8_t>(0x3F & (has_z ? src_b_row_stride : 0)), .clr = 0, .cr = has_z},
    ...
    addr_mod_t {
        ...
        .srcb = {.incr = static_cast<std::uint8_t>(0x3F & k_src_b), .clr = 0, .cr = has_k},
        .dest = {.incr = 0, .clr = 0, .cr = 1},
    }
        .set(ADDR_MOD_3);
...
/**
 * @brief Returns the address-modifier slot for the no-MOP fidelity-phase closing MVMUL.
 *
 * @tparam ENABLE_2X_FORMAT: Selects the MXFP4_2x instruction sequence.
 */
template <bool ENABLE_2X_FORMAT>
...
/**
 * @brief Returns the address-modifier slot for the no-MOP tile-closing MVMUL.
 *
 * @tparam ENABLE_2X_FORMAT: Selects the MXFP4_2x instruction sequence.
 */
template <bool ENABLE_2X_FORMAT>
